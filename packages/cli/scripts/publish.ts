  ...
  JSON.stringify(
    {
      ...
      // Don't include React exports/dependencies for binary-only package
    },
    null,
    2
  )
...
// Create the React hooks package
await $`mkdir -p ./dist/${pkg.name}-react`;
await $`cp -r ./dist/src ./dist/${pkg.name}-react/`;
await $`cp ./templates ./dist/${pkg.name}-react/ -r`;

await Bun.file(`./dist/${pkg.name}-react/package.json`).write(
  JSON.stringify(
    {
      name: `${pkg.name}-react`,
      version,
      description: `${pkg.description} - React hooks`,
      keywords: [...pkg.keywords, 'react', 'hooks'],
      author: pkg.author,
      license: pkg.license,
      repository: pkg.repository,
      exports: {
        '.': {
          types: './src/react-hooks.d.ts',
          default: './src/react-hooks.js',
        },
      },
      types: './src/react-hooks.d.ts',
...
if (!dry) await $`cd ./dist/${pkg.name}-react && bun publish --access public --tag ${npmTag}`;
